...
const Coinfavo = require("../models/favoriteCoin");
...
exports.fetchCoins = async (req, res) => {
  try {
    ...
    const oneHourInMillis = 5 * 60 * 1000;
...
exports.favoriteCoins = async (req, res) => {
  const userId = req.user.userId;
  console.log(userId);

  try {
    const userFromDB = await user.findById(userId);
    console.log("llllllllllllll" + userFromDB);
    ...
    // Extraire les noms des cryptos
    const cryptomonnie = userFromDB.cryptocurrencies.filter(
      (defaultCurrency) => defaultCurrency
    );

    if (cryptomonnie.length === 0) {
      return res
        .status(405)
        .json({ message: "vous n'avez pas de favorits pour le moment ." });
    }

    const devisecrypto = userFromDB.defaultCurrency;
    ...
    if (currentTime - lastUpdateTimestamp < oneHourInMillis) {
      const storedData = await Coinfavo.find();
      res.status(200).json(storedData);
      return;
    }
    ...
    // Construire l'objet de paramètres pour la requête Coingecko
    const coingeckoParams = {
      vs_currency: devisecrypto,
      order: "market_cap_desc",
      sparkline: false,
      ids: cryptomonnie.join(","), // Convertir les noms en une chaîne séparée par des virgules pour la requête Coingecko
    };
    ...
    await Coinfavo.insertMany(selectedData);
